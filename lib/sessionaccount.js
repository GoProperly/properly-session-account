...
const STAGING_GRANT_CONFIG = {
  ...
  signOut: "https://stagingauth.goproperly.com/logout",
...
export class SessionAccount {
  ...
  getJWT() {
    ...
    if (idToken) {
      //We have the idToken in a cookie, resolve immediately
    ...
    const promiseToReturn = apiCompletionPromise.then((response) => {
      ...
      //TODO: this is a bad spot for this call, it should be done after the first real login, but doesn't need to be
      //redone on each exchange of tokens.
      this.performCheckLoginToServer(idTokenFromAPI);
