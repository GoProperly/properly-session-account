...
const STAGING_GRANT_PROPERLY_DISCO_CONFIG = {
  login: 'https://authhandler-staging.properly.ca/%1$s?mode=prop-disco&redirectUrl=%2$s',
  exchangeTokens: 'https://authhandler-staging.properly.ca/exchangeTokens?mode=prop-disco',
  signOut: "https://authhandler-staging.properly.ca/startSignOut?mode=prop-disco",
  checkLoginTemplate: 'https://2tpc2k1y2l.execute-api.us-east-1.amazonaws.com/staging/account/checklogin/%1$s',
  role: 'jczqf4GgkEKd1bxtTgpJOg',
};

const PROD_GRANT_PROPERLY_DISCO_CONFIG = {
  login: 'https://authhandler.properly.ca/%1$s?mode=prop-disco&redirectUrl=%2$s',
  exchangeTokens: 'https://authhandler.properly.ca/exchangeTokens?mode=prop-disco',
  signOut: 'https://authhandler.properly.ca/startSignOut?mode=prop-disco',
  checkLoginTemplate: 'https://w5gv3a4n5e.execute-api.us-east-1.amazonaws.com/prod/account/checklogin/%1$s',
  role: 'jczqf4GgkEKd1bxtTgpJOg',
};
...
configMap['buy.properly.ca'] = PROD_GRANT_PROPERLY_DISCO_CONFIG;
configMap['dev.properly.ca'] = STAGING_GRANT_PROPERLY_DISCO_CONFIG;
configMap['buy-staging.properly.ca'] = STAGING_GRANT_PROPERLY_DISCO_CONFIG;

//Merged subdir site
configMap['wwwsubdir.properly.ca'] = PROD_GRANT_PROPERLY_DISCO_CONFIG;
configMap['wwwsubdir-staging.properly.ca'] = STAGING_GRANT_PROPERLY_DISCO_CONFIG;
configMap['www.properly.ca'] = PROD_GRANT_PROPERLY_DISCO_CONFIG;
configMap['www-staging.properly.ca'] = STAGING_GRANT_PROPERLY_DISCO_CONFIG;
...
class SessionAccount {
  ...
  getJWT() {
    ...
    const promiseToReturn = apiCompletionPromise
      .then((response) => {
