...
export class SessionAccount {
        ...
        this.auth.userhandler = {
            onSuccess: (result) => {
                ...
                if (this.operatingInHandleLoginContext) {
                    this.performNewLoginActivities();
                }
                else {
    ...
    performCheckLoginToServer() {

        const checkLoginTemplate = this.currentConfig.checkLoginTemplate

        const userId = UserInfoStash.getUserId();

        let uriToUse = Sprintf(checkLoginTemplate, userId);
        ...
        let promiseToReturn = apiCompletionPromise.then(function(response) {
            console.log('set home info back.');
            console.log('Response for %o is: %o ', offerId, response);
            ...
            console.log(Sprintf("Original ID %1$s, ID from Server %2$s", userId, serverUserId) );
    ...
    performNewLoginActivities() {
        console.log("callback for successful login from loginhandler");

        let apiCallPromise = this.performCheckLoginToServer();
